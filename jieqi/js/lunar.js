...
var oba = {//公历基础构件
	...
	'0520.国际母亲节', '0530I全国助残日', '0630.父亲节', '0730.被奴役国家周', '0932I国际和平日', '0940.国际聋人节 世界儿童日', '0950I世界海事日', '1011.国际住房日', '1013I国际减轻自然灾害日(减灾日)', '1144I感恩节'),
	sFtv : '', //假日表,由init初始化
	...
	init : function() {//初始化
		...
		+ '15I1983-9999消费者权益日,17.中国国医节,17.国际航海日,21.世界森林日,21.消除种族歧视国际日,21.世界儿歌日,22I世界水日,' + '23I世界气象日,24.1982-9999世界防治结核病日,25.全国中小学生安全教育日,30.巴勒斯坦国土日|' + '01I1564-9999愚人节,01.全国爱国卫生运动月(四月),01.税收宣传月(四月),07I世界卫生日,22I世界地球日,23.世界图书和版权日,24.亚非新闻工作者日|'//4月

		+ '01#1889-9999劳动节,04I青年节,05.碘缺乏病防治日,08.世界红十字日,12I国际护士节,15I国际家庭日,17.国际电信日,18.国际博物馆日,'//5月
		+ '20.全国学生营养日,23.国际牛奶日,31I世界无烟日|' + '01I1925-9999国际儿童节,05.世界环境保护日,06.全国爱眼日,17.防治荒漠化和干旱日,23.国际奥林匹克日,25.全国土地日,26I国际禁毒日|'//6月
...
var obb = {//农历基础构件
	...
	mingLiBaZi : function(jd, J, ob) {//命理八字计算。jd为格林尼治UT(J2000起算),J为本地经度,返回在物件ob中
		...
		ob.bz_js = this.Gan[v % 10] + this.Zhi[v % 12]; v -= SC, ob.bz_JS = ''; //全天纪时表
		for( i = 0; i < 13; i++) {//一天中包含有13个纪时
			c = this.Gan[(v + i) % 10] + this.Zhi[(v + i) % 12];
			//各时辰的八字
			if(SC == i)
				ob.bz_js = c, c = '<font color=red>' + c + '</font>'
			//红色显示这时辰
			ob.bz_JS += ( i ? ' ' : '') + c;
		}
	},
	...
	this.yueLiCalc = function(By, Bm) {
		...
		for( i = 0, j = 0; i < Bdn; i++) {
			...
			if(ob.d0 == SSQ.HS[mk] || ob.d0 == Bd0) {//月的信息
				...
				ob2 = this.lun[i - 1]; ob.Lmc = ob2.Lmc, ob.Ldn = ob2.Ldn;ob.Lleap = ob2.Lleap, ob.Lmc2 = ob2.Lmc2;
			}
