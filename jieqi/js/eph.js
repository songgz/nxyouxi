...
//大距计算
	...
	function lineEll(x1, y1, z1, x2, y2, z2, e, r) {//求空间两点连线与地球的交点(靠近点x1的交点)
		...
		if(B < 0)
			D = -D;
		//只求靠近x1的交点
		t = (-B + D) / A; p.x = x1 + dx * t, p.y = y1 + dy * t, p.z = z1 + dz * t;
		R = sqrt(dx * dx + dy * dy + dz * dz); p.R1 = R * abs(t), p.R2 = R * abs(t - 1); //R1,R2分别为x1,x2到交点的距离
		return p;
	}
...
****/
	...
	function lineOvl(x1, y1, dx, dy, r, ba) {
		...
		if(!D)
			...
			p.n = 2;
		D = Math.sqrt(D); t1 = (-B + D) / A, t2 = (-B - D) / A;
...
var ysPL = { //月食快速计算器
	...
	lecXY:function(jd,re) {//日月黄经纬差转为日面中心直角坐标(用于月食)
		...
		re.y = zm[1] + zs[1]; re.mr = re.e_mRad / rad, re.er = re.eShadow / rad, re.Er = re.eShadow2 / rad;
		re.t = jd;
	},
...
function ecFast(jd) {//快速日食搜索,jd为朔时间(J2000起算的儒略日数,不必很精确)
	...
	//平朔时间
	re.jd = re.jdSuo = t * 36525; t2 = t * t, t3 = t2 * t, t4 = t3 * t;
	L = (93.2720993 + 483202.0175273 * t - 0.0034029 * t2 - t3 / 3526000 + t4 / 863310000 ) / 180 * Math.PI; re.ac = 1, re.lx = 'N';
	...
	mR /= 6378.1366; t = jd / 365250, t2 = t * t, t3 = t2 * t;
	//误0.0002AU
	sR = 10001399//日地距离
	+ 167070 * Math.cos(3.098464 + 6283.07585 * t) + 1396 * Math.cos(3.0552 + 12566.1517 * t) + 10302 * Math.cos(1.10749 + 6283.07585 * t) * t + 172 * Math.cos(1.064 + 12566.152 * t) * t + 436 * Math.cos(5.785 + 6283.076 * t) * t2 + 14 * Math.cos(4.27 + 6283.08 * t) * t3;
	sR *= 1.49597870691 / 6378.1366 * 10;

	//经纬速度
	t = jd / 36525;
	vL = 7771//月日黄经差速度
	- 914 * Math.sin(0.785 + 8328.6914 * t) - 179 * Math.sin(2.543 + 15542.7543 * t) - 160 * Math.sin(0.187 + 7214.0629 * t);
	vB = -755 * Math.sin(0.057 + 8433.4662 * t)//月亮黄纬速度
	- 82 * Math.sin(2.413 + 16762.1576 * t);
	vR = -27299 * Math.sin(5.497 + 8328.691425 * t) - 4184 * Math.sin(4.900 + 7214.06287 * t) - 7204 * Math.sin(0.972 + 15542.75429 * t); vL /= 36525, vB /= 36525, vR /= 36525; //每日速度
...
var rsGS={
	...
	init:function(jd,n) {//创建插值表(根数表)
		...
		for( i = 0; i < n; i++) {//插值点范围不要超过360度(约1个月)
			...
			k = i * 9; a[k + 0] = S[0], a[k + 1] = S[1], a[k + 2] = S[2]; //存入插值表
	...
	bseM:function(jd) {//月亮的贝塞尔坐标
		var a = this.cd2bse(this.chazhi(jd, 1), this.chazhi(jd, 2)); a[0] /= cs_rEar, a[1] /= cs_rEar, a[2] /= cs_rEar;
		return a;
	},
	...
	qrd:function(jd,dx,dy, fs) {//求切入点
		...
		var t = (d * d - x * x - y * y) / (dx * x + dy * y) / 2; x += t * dx, y += t * dy, jd += t;
	...
	nanbei:function(M,vx0,vy0, h, r,I) {
		...
		for( i = 0; i < 3; i++) {
			...
			v = Math.sqrt(vx * vx + vy * vy); sinA = h * vy / v, cosA = h * vx / v; x = M[0] - r * sinA, y = M[1] + r * cosA;
		}
	...
	jieX:function(jd) {
		...
		re.p1 = new Array(), re.p2 = new Array(), re.p3 = new Array(), re.p4 = new Array();re.q1 = new Array(), re.q2 = new Array(), re.q3 = new Array(), re.q4 = new Array();re.L1 = new Array(), re.L2 = new Array(), re.L3 = new Array(), re.L4 = new Array();re.L5 = new Array(), re.L6 = new Array(); //0.5食分线
		re.L0 = new Array();
		//中心线
	...
	jieX2:function (jd) {//jd力学时
		...
		for( i = 0; i < N; i++) {//第0和第N点是同一点，可形成一个环，但不必计算，因为第0点可能在界外而无效
			s = i / N * pi2;
			var cosS = cos(s), sinS = sin(s); X = M[0] + cs_k * cosS, Y = M[1] + cs_k * sinS;
			//本影
			...
			this.push(p, p3);
		}p1[p1.length] = p1[0], p1[p1.length] = p1[1];p2[p2.length] = p2[0], p2[p2.length] = p2[1];p3[p3.length] = p3[0], p3[p3.length] = p3[1];re.p1 = p1, re.p2 = p2, re.p3 = p3;
		return re;
	},
